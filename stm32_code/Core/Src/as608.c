...
/*****************************************
The functions below are functions
which were created to used in
the smart doorlock project
*****************************************/
// Get the maximum fingerprint ID
uint16_t Get_MaxFingerprintID(void) {
    SearchResult searchResult;
    uint8_t ensure;
    uint16_t maxID = 0;

    // Start searching from page 0 to the maximum possible page (e.g., 100 pages).
    for (uint16_t page = 0; page < 100; page++) {
        ensure = PS_HighSpeedSearch(CharBuffer1, page, 1, &searchResult);
        if (ensure == 0x00) {
            // If a fingerprint is found on the current page, update the max ID.
            if (searchResult.pageID > maxID) {
                maxID = searchResult.pageID;
            }
        } else if (ensure == 0x09) {
            // If no fingerprint is found on this page, skip to the next page.
            continue;
        }
    }
    return maxID; // Return the highest ID found in the fingerprint library.
}
