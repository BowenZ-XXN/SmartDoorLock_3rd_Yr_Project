...
fm.register(35,fm.fpioa.UART1_TX)#串口引脚映射
fm.register(34,fm.fpioa.UART1_RX)#这两个引脚是可以任意修改的
...
def on_timer(timer):  #回调函数
    ...
    if data!=None:
        ...
        if data == b'A':
            ...
            check = 1#代表存储人脸特征
            save = 1  #存到SD卡中
        elif data == b'D':  # 如果接收到删除全部命令
            #delete = 1  # 调用删除全部人脸特征的函数
            delete_all_faces()
...
def delete_all_faces():
    global record_ftrs
    global names
    record_ftrs = []  # 清空已存的人脸特征列表
    print("Deleted all stored face features from memory.")

    # 更新 SD 卡文件，清空文件内容
    try:
        with open('/sd/features.txt', 'w') as f:
            f.truncate(0)  # 清空文件内容
        print("Deleted all face features from SD card.")
    except Exception as e:
        print("Error while deleting from SD card: {e}")
...
task_fd = kpu.load("/sd/FaceDetection.smodel")         # 从flash 0x200000 加载人脸检测模型
task_ld = kpu.load("/sd/FaceLandmarkDetection.smodel") # 从flash 0x300000 加载人脸五点关键点检测模型
task_fe = kpu.load("/sd/FeatureExtraction.smodel")     # 从flash 0x400000 加载人脸196维特征值模型
